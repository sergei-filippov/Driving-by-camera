...
void loop() {
  ...
  for (y = lineofcount; y < H ; y += 66) {
    ...
    //--------------------------------------------------------------------//to move better // xshift
    if (((xendm[y] + xstartm[y]) / 2) > (W / 2) + 10) { // more than a middle+10
      midx = ((xendm[y] + xstartm[y]) / 2) - xshift;
    } else if (((xendm[y] + xstartm[y]) / 2) < (W / 2) - 10) {
      midx = ((xendm[y] + xstartm[y]) / 2) + xshift;
    } else {
      midx = ((xendm[y] + xstartm[y]) / 2);
    }
    //-------*-----------------------------------------------//
    /* if (y >= 20) {
    ...
    midx = ((xendm[y] + xstartm[y]) / 2);
    tgangle[y] = atan(1.0 * (midx - (W / 2)) / ((H - y) + 90));
