...
find_black_obj(){   //начать с первой линии и в случае встречи черного обьекта
                    //пройтись по его границам. считаем и столбики и строчкм через раз
  int y=1;
  int nodes[4][2];  // x-y
  int flag_node=0;   // how many nodes we have counted  // start from top (from left to right) then goes in clockwise direction
  for(int y = 0;y < W;y++){    //find the first line (2 nodes on the top)
    for(int x = 0;x < W;x+++){
      bool c = tv.get_pixel(x,y);

      if(c && flagnode==0){     // если что-то черное нашли
        nodes[0][0]=x;
        nodes[0][1]=y;
        flagnode++;
      }
...
void loop() {
  ...
  tv.capture();
  //tv.fill(INVERT);
  find_black_obj();  //возвращает углы четырехугольника (треугольника)
